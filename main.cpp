...
UDP_Server::UDP_Server() {
    ...
    while (true) {
        ...
        recvfrom(sockSrv_server, recvBuf, 2000, 0, (SOCKADDR *) &addrClient_server, &len_server);
        string recvBuf_s = recvBuf;
        if (recvBuf_s.find("cmd=1001,;") == 0) {
            //232
            if (recvBuf_s.substr(16, 1) == "0") {
                string MeterPosition = recvBuf_s.substr(14, 1);
                string cut = recvBuf_s.substr(18);
//todo            this->mywrite(cut);
                cout << "send to port:" << cut << endl;
            }
        } else {
            if (recvBuf_s.find("cmd=0104") == 0) {
                ...
                UDPServerToClient.emplace_back(recvBuf);
            }
        }
        if (!UDPClientToServer.empty()) {
            for (int i = 0; i < UDPClientToServer.size(); ++i) {
                rec = UDPClientToServer[i];
                strncpy(sendBuf_server, rec.c_str(), rec.length() + 1);
                sendto(sockSrv_server, sendBuf_server, strlen(sendBuf_server), 0, (SOCKADDR *) &addrClient_server,
                       len_server);
...
UDP_Client::UDP_Client() {
    ...
    while (true) {
        ...
        if (!UDPServerToClient.empty()){
            for (auto text : UDPServerToClient) {
                sendto(sockSrv, text.c_str(), strlen(text.c_str()) + 1, 0, (SOCKADDR *) &addrSrv, len);
            }
        }
    }
}
...
int main() {
    ...
    serial_list.insert(pair<string, string>("1", "3"));
...
//    serial_list.insert(pair<int,string>(5,"22"));
//    serial_list.insert(pair<int,string>(6,"23"));
//    serial_list.insert(pair<int,string>(7,"24"));
//    serial_list.insert(pair<int,string>(8,"25"));
